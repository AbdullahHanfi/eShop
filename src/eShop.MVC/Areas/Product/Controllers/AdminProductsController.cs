...
namespace eShop.MVC.Areas.Product.Controllers {
    ...
    public class AdminProductsController : Controller {
        ...
        public async Task<IActionResult> Index(int? page) {
            ...
            var model = _unitOfWork.Products.Include(e => e.Images).Skip((pageNumber - 1) * Page.MaxElementsInPage).Take(Page.MaxElementsInPage).ToList();
        ...
        public async Task<IActionResult> Add(CreateProductViewModel model) {
            ...
            catch (Exception){ _toastNotification.AddWarningToastMessage("Error: problem in database!"); }
        ...
        public async Task<IActionResult> Delete(Guid Id) {
            if (Id == Guid.Empty){ return BadRequest("Product ID is required."); }
            var product = _unitOfWork.Products.GetById(Id);

            if (product is null || product.IsDeleted){ return NotFound($"No product found for ID {Id}."); }

            product.IsDeleted = true;
            _unitOfWork.Products.Update(product);

            try{ _unitOfWork.Complete(); }
            catch (Exception){ return StatusCode(StatusCodes.Status500InternalServerError, "An error occurred while deleting the product."); }
