...
namespace eShop.DAL.Repositories.implementation {
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : BaseEntity {
        ...
        protected string GetTableName(Type entityType) {
            ...
            if (efEntityType == null){ throw new InvalidOperationException($"Entity type {entityType.Name} is not part of the model."); }

            var tableName = efEntityType.GetTableName();

            if (string.IsNullOrEmpty(tableName)){ throw new InvalidOperationException($"No table name found for entity type {entityType.Name}."); }
        ...
        public TEntity Find(Expression<Func<TEntity, bool>> criteria, params Expression<Func<TEntity, object>>[] includes) {
            ...
            if (includes != null)
                foreach (var include in includes){ Data = Data.Include(include); }
